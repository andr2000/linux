	...
	struct sdev_pcm_stream_info *stream)
{
	...
	req->op.open.gref_directory =
		sh_buf_get_dir_start(&stream->sh_buf);
	...
	snd_pcm_uframes_t len)
{
	...
	req->op.rw.offset = 0;
	return sdrv_be_stream_do_io(stream->evt_chnl, req, flags);
}
		...
		int num_streams)
{
	...
	for (d = 0; d < plat_data->cfg_card.num_pcm_instances; d++) {
	...
	while ((cur_rate = strsep(&list, XENSND_LIST_SEPARATOR))) {
		for (i = 0; i < ARRAY_SIZE(cfg_hw_supported_rates); i++)
			if (!strncasecmp(cur_rate,
				...
				cur_mask =
					cfg_hw_supported_rates[i].mask;
				cur_value =
					cfg_hw_supported_rates[i].value;
				rates |= cur_mask;
	...
	struct sdev_card_plat_data *plat_data, int *stream_idx)
{
	...
	cfg_pcm_hw(xb_dev->nodename, &sdrv_pcm_hardware_def,
...
static int xdrv_be_on_initwait(struct xdrv_info *drv_info)
{
	...
	/* 
	 * pass stream index, so it is incremented when creating streams
	 */
	stream_idx = 0;
	/* 
