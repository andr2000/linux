...
{
	struct card_info *card_info;
	struct xen_snd_front_cfg *platdata;
	struct snd_card *card;
	int ret, i;
	...
	card_info->front_info = platdata->front_info;
	card_info->card = card;
...
void xen_snd_front_alsa_cleanup(struct xen_snd_front_info *front_info)
{
	if (!front_info->snd_pdrv_registered)
		return;

	if (front_info->snd_pdev) {
		struct platform_device *snd_drv_pdev;

		snd_drv_pdev = front_info->snd_pdev;
		if (snd_drv_pdev)
			platform_device_unregister(snd_drv_pdev);
	}

	platform_driver_unregister(&snd_drv_info);
	front_info->snd_pdrv_registered = false;
}
...
int xen_snd_front_alsa_init(struct xen_snd_front_info *front_info)
{
	...
	snd_drv_pdev = platform_device_register_data(NULL,
			XENSND_DRIVER_NAME, 0, &front_info->cfg,
			sizeof(front_info->cfg));
	if (IS_ERR_OR_NULL(snd_drv_pdev))
		goto fail;

	front_info->snd_pdev = snd_drv_pdev;
	return 0;
